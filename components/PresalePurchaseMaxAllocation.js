...
import { PRESALE_ADDRESS, DAI_ADDRESS, MAX_TOKEN_ALLOCATION } from "../core/constants";
import { usePresale } from "../util/presale";

export default function PresalePurchaseMaxAllocation({
  handleBack,
  handleDisconnect,
  amountPurchased
}) {
  const { presaleEnded, userRemainingAllocation } = usePresale({ 
    presaleInterface, 
  ...
  if(!presaleEnded && (userRemainingAllocation === 0)) {
    return (
